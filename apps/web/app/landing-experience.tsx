...
export function LandingExperience() {
  ...
  useEffect(() => {
    ...
    const paint = () => {
      ...
      let current = 0;
      root.current?.querySelectorAll<HTMLElement>(".story-chapter").forEach((section, index) => {
        if (section.getBoundingClientRect().top < window.innerHeight * 0.55) current = index;
      });
  ...
  return <main ref={root} className={`lunar-story${reducedMotion ? " motion-reduced" : ""}`} id="top">
    ...
    <div className="story-progress" ref={progress} aria-hidden="true" />
    ...
    <section className="story-hero" aria-labelledby="story-title">
      <div className="story-cosmos"><Suspense fallback={<div className="story-moon-fallback" />}><LunarScene reducedMotion={reducedMotion} /></Suspense></div>
      ...
      <div className="story-moon-caption" aria-hidden="true"><span>LUNARVEIL / THE PRIVATE MARKET LAYER</span><i /><span>INTENT, UNDER COVER.</span></div>
      <div className="story-hero-bottom"><a href="#journey"><span className="scroll-track" /> Scroll to uncover</a><span>A new perspective on exchange</span><span>01 — 05</span></div>
    </section>
    ...
    <div className="story-journey">
      <aside className="story-chapter-nav" aria-label="Product story chapters">{chapters.map((chapter, index) => <a key={chapter.id} href={`#${chapter.id}`} aria-current={active === index ? "step" : undefined}><span>0{index + 1}</span><i />{chapter.name}</a>)}</aside>
      <div>{chapters.map((chapter, index) => <section key={chapter.id} id={chapter.id} className="story-chapter" aria-labelledby={`${chapter.id}-title`}>
        ...
        <div className="story-diagram" data-reveal role="img" aria-label={index === 0 ? "An encrypted order hides price, size and strategy" : index === 1 ? "Buy and sell orders gather into a batch with one clearing price" : "Compact checks the frozen order set and published rule; altered allocations are rejected"}>
          ...
          {index === 0 ? <div className="sealed-visual"><div className="seal-orbit" /><div className="seal-orbit orbit-b" /><div className="sealed-card"><span className="sealed-label">PRIVATE LIMIT ORDER</span>{["Price", "Size", "Strategy"].map(label => <div key={label}><span>{label}</span><b>••••••</b></div>)}<footer>◇ ENCRYPTED INTENT</footer></div></div>
          ...
          <div className="diagram-bottom"><span>{["PUBLIC CHAIN SEES A COMMITMENT", "DETERMINISTIC BY DESIGN", "AN OUTCOME YOU CAN CHECK"][index]}</span><span aria-hidden="true">↗</span></div>
        </div>
      </section>)}</div>
    </div>
    ...
    <section className="story-finale" aria-labelledby="finale-title"><div className="finale-orbit" aria-hidden="true" /><div data-reveal><p className="story-kicker"><span className="story-status-dot" /> BUILT ON MIDNIGHT</p><h2 id="finale-title">A clearer market.<br /><em>A quieter footprint.</em></h2><p>Explore the next chapter of private exchange.</p><a className="story-button" href="/markets">Enter Lunarveil ↗</a><small>Preview network · Protocol in development</small></div></section>
