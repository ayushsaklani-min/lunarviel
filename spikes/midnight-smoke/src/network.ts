...
export type ContractDeploymentId =
  | 'hello-world'
...
export interface NetworkState {
  ...
  contractDeployments?: Partial<
    Record<NetworkId, Partial<Record<ContractDeploymentId, DeploymentRecord>>>
  >;
}
...
export function loadState(opts: FsOptions = {}): NetworkState | null {
  ...
  const candidate = parsed as {
    ...
    deployments?: NetworkState['deployments'];
    contractDeployments?: NetworkState['contractDeployments'];
  };
  ...
  if (candidate.version === 1) {
    // V1 stored wallet seeds. Migrate only public deployment metadata and
    // atomically overwrite the file so seed material is scrubbed.
    const migrated: NetworkState = {
      version: STATE_VERSION,
      activeNetwork: candidate.activeNetwork,
      deployments: candidate.deployments ?? {},
    };
    saveState(migrated, opts);
    return migrated;
  }
...
export function getWalletSeed(network: NetworkId, opts: SeedOptions = {}): string {
  const env = opts.env ?? process.env;
  const fromEnv = env.MIDNIGHT_WALLET_SEED;
  if (!fromEnv) {
    throw new Error(
      `MIDNIGHT_WALLET_SEED is required for ${network}. Supply it through your shell or secret manager; it is never stored by Lunarveil.`,
    );
  }
  if (!/^[0-9a-fA-F]{64}$/u.test(fromEnv)) {
    throw new Error('MIDNIGHT_WALLET_SEED must be exactly 32 bytes encoded as 64 hexadecimal characters.');
  }
...
export function getContractDeployment(
  network: NetworkId,
  contractId: ContractDeploymentId,
  opts: FsOptions = {},
): DeploymentRecord | null {
  const state = loadState(opts);
  return state?.contractDeployments?.[network]?.[contractId] ?? null;
}
...
export function recordContractDeployment(
  ...
  next.contractDeployments = {
    ...next.contractDeployments,
    [network]: {
      ...next.contractDeployments?.[network],
      [contractId]: { address, deployer, deployedAt: new Date().toISOString() },
    },
  };
  saveState(next, { cwd });
}
...
function cliMain(argv: string[]): number {
  ...
  process.stdout.write('Set MIDNIGHT_WALLET_SEED through your shell or secret manager before wallet commands.\n');
  return 0;
}
