/**
 * End-to-end smoke check for midnight-smoke.
 *
...
import { getDeployment, getWalletSeed, resolveNetwork } from '../src/network';
...
const SEED = getWalletSeed(network);
...
async function main() {
  ...
  // 3. Reconnect to the deployed contract and submit one real circuit call.
  let deployed: any;
  try {
  ...
  const expectedMessage = 'Lunarveil Milestone 0 smoke';
  const callResult = await deployed.callTx.storeMessage(expectedMessage);

  // 4. Read and decode the on-chain contract state. This verifies that the
  // transaction was accepted and the indexer exposes the updated ledger value.
  ...
  const actualMessage = Buffer.from(ledgerState.message).toString();
  if (actualMessage !== expectedMessage) {
    await walletCtx.wallet.stop();
    fail(`ledger message mismatch: expected ${JSON.stringify(expectedMessage)}, got ${JSON.stringify(actualMessage)}`);
  }

  console.log(`✅ e2e-check passed`);
  console.log(`   contractAddress: ${deployment.address}`);
  console.log(`   network:         ${network}`);
  console.log(`   transactionId:   ${callResult.public.txId}`);
  console.log(`   blockHeight:     ${callResult.public.blockHeight}`);
  console.log(`   ledgerMessage:   ${actualMessage}`);
